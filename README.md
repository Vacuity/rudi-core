...
* Metrics - this subnet collects metrics pertaining to nodes and events. For example, a word is mapped to its branching factor (the number of encountered next word), as well as the network-wide string event tree, and the number of occurances of each node. The tree for string events is shallow, but wide. The branching factor for the first token is large quite large, but the average branching factor for subsequent tokens in the event decreases rapidly. This property is taken into account during assignment of nodes.
...
##<a href="security"/>Security Model
