...
6. [Additional Node Services](#additionalServices)
...
##<a name="additionalServices"/>Additional Node Services

Other types of distributed services may be built on top of Pastry to assists the RUDI network with executing its function.  A node is configured to be a worker in the worker pool of these services by adding the service IRI to the `rudi.workerpools` settings property. This causes the node (nodeId) to join the workerpool's work load. The assignment of nodeIds is worker pool specific. Typically it is computed as the SHA-1 hash of the node's IP address or its public key. Worker pool objects (e.g. a namespace prefix for the Namespace Prefix Service) are assigned unique, uniform random identifiers (objIds) and its requests are handled by the k (where k >= 1) nodes with nodeIds numerically closest to the objId. The value k reflects the worker pool's desired degree of replication for its objects. Object load balancing is implemented by the worker pool code.

Some example worker pools are listed below.

* **Namespace Prefix Server** - this instance of the DHT maps XML namespace qualified names to their IRI prefixes. RUDI routinely retrieves the list of namespaces prefixes it knows about and compares the list to those reported on the previous check. The diff is added to the known prefix list and the new namespaces are stored in the DHT. Each namespace mapping has associated with it the list of nodes who know about the namespace mapping. A remove operation removes from the sources list the node that invoked the remove operation. When all nodes have been removed from the sources list, the namespace mapping is removed from the Index.
* **Web Service Reply Cache** - nodes may make use of a shared, distributed Web Service Reply Cache similar to [Squirrel](http://www.freepastry.org/PAST/squirrel.pdf).
* **SPARQL P2P Endpoint** - a node can accept a SPARQL request message and reply with a QueryResult message. The RUDI HTTPSensor exposes its peer's SPARQL endpoints at [http://rudi-host/sparql/nodeId](http://rudi-host/sparql/nodeId]).
* **Metrics** - this subnet collects metrics pertaining to nodes and events. For example, a token of a string event (i.e. a free-form query or a LDPa) is mapped to its branching factor (the number of encountered next words). This key is a node in the network-wide string event tree, and is associated with the number of instances of the word at that node. The tree for string events is shallow, but wide. The branching factor for the first token is large quite large, but the average branching factor for subsequent tokens in the event string decreases rapidly. This property is taken into account during assignment of nodeIds for the purpose of load-balancing.

#<a name="programming"/>VI Programming
The RUDI network replies to events with behavior that is useful to its clients, like performing actions or returning information. This behavior is “smart” in as much as it is influenced by semantics encapsulated by the event. The network harvests intelligence by dismantling its problem set into a "jigsaw puzzle", the solution to which many individuals can contribute small portions, perhaps three or four fitted pieces each. These individuals give the portions they solve a unique identifier like “034” and add to it an terse assembly instruction such as “pieces 034 and 091 fit”. The individual then leaves the pieces scattered and a computer program is later called to solve some portion of the puzzle. Though it is merely doing what programs have always been good at (following opaque, but well-defined, procedures), to an unwitting observer it appears the program has intelligently assembled the jigsaw puzzle. This [ELIZA-style](https://en.wikipedia.org/wiki/ELIZA) approach of leveraging many self-contained, stimulus/response scripts, or microprocedures, is quite effective when it works, but is often plauged by the brittleness barrier. Intelligence is exhibited within the agent's input domain but the domain is easily evaded by the end-user. A certain critical mass of microprocedures is required to overcome this barrier in non-trivial contexts. With the introduction of networking, morsels of intelligence from of a large army of individuals can be brought to bear to reach this critical mass, thus overcoming the brittleness barrier. Technical risk is eliminated by transferring the problem of machine intelligence from the domain of theory and the province of esoteric algorithms to the well charted domains of protocol design and networking. The logic tier has been evacuated. *How* the nodes are linked has become the intelligence. The algorithm has been [ephemeralized](http://live.dbpedia.org/page/Ephemeralization) by a protocol. The network has replaced the role of the logic board. The resulting intelligence is all smoke and mirrors. Pay no attention to the man behind the curtain. This is the vacuous model, which facilitates virtualization of the smart agent by decoupling the intelligence and the agent that consumes and applies it. In such a model the end-user is removed no more than one degree from geniune intellect. The agent is analogous to a puppet serving as a vacuous intermediary (provides a faithful mirror) for its master's behavior. The puppeteer serves as the agent's primary source of intelligence. An agent who relies heavily on a learning set is not vacuous, but would set the end-user at least two degrees from geniune intellect (human generates learning algorithm, learning algorithm generates the intelligence). With RUDI, the network intercedes for *virtual intelligence*, where a node's behavior is analogous to a film comprised of still frames, each of which depicts the performance of some one of an innumerable host of puppeteers.

There is a territory of phenomena whose manifestations are caused by indeterminate factors. This class of events is embraced by the NP-complete set. Problems involving such phenomena have solutions that are not conducive to formal expression (e.g. the event cannot be generated by a grammar *a priori*, and must instead be explicitly enumerated). This limits the production of event recognition logic at the network's sensory layer. For this reason, virtual intelligence is an inescapable simulation of the real thing, there are a set of problems (encapsulated as events) for which the network cannot serve as intermediary between problem-bearer and problem-solver. Something is neccessarily absent in the *imitator* (the network) that is present at its source (the author swarm), and this limitation is inversely proportional to the expressive power of the language encapsulating the sensory layer. VI is a sharp departure from AI, which produces a *duplicate* of the human intellect in a material energy mechanism (e.g. a microprocessor or a network of such). AI is therefore non-vacuous. VI approaches non-vacuity to the degree of its autonomy, and approaches AI to the degree of its performance in a [Turing test](http://dbpedia.org/page/Turing_test).

#### REST Service API

The documentation for the RUDI Rest API is located at [http://rudi.vi/swagger-ui.html](http://rudi.vi/swagger-ui.html).
...
**Channels** are virtual communications conduits implemented as SPARQL graph contexts within the RUDI index. The two primary types of channels are input and response. Channels are RESTfully navigated (the user requests some data to start with, then browses the results and requests the "rest" as needed). To enable this mode of browsing, service responses are converted to hyperdata (the resources are linked to resources contained elsewhere in the great global graph).

##<a name="sponsors"/>Sponsors

Sponsor messages are delivered by the network to the origin node exclusively through trusted Event Handlers. Trust in the network is crypto-identity based and is incremented by complaint-free deliveries. Complaints decrement trust. Complaints are registered though the UI using a "report {abuse|spam|offensive content}" feature. All ads must be delivered to the Sponsor channel by a trusted Event Handler (one that has responded to the user's request with useful behavior). In this scheme, Sponsors register their content to a Sponsors worker pool. Prior to notifying the origin node of result, but after receipt dispatch reply, event handlers negotiate conditions for the ad with the Sponsor worker pool. The Sponsor worker pool then emits a sponsor message, by matching against the semantics of either the input or response channel. Since all transactions are stored in the DHT, payments can be made periodically by the Sponsor worker pool on behalf of the sponsor, where the transaction logs from Event Handler to origin is the community-vouched invoice. To this end, the network hosts a forked block-chain.
...
| :------------------- | :--------------------------------------- | :----------------------: | :---------: |
| **via:Listener**     | A RUDI listener. IRI dereference is attempted on this resource. |                          |      n      |
...
| **via:EventHandler** | accepts an event to disptach; the captured data is passed to via:call values via ${index} placeholders, where 'index' is the index of the captured datum. IRI dereference is attempted on this resource. |                          |             |
...
| -via:invoke          | the Java implementation of `ai.vacuity.rudi.adaptors.interfaces.IResponseModule` which handles the event and whose `run()` method is called (after the results of the dispatch or query are consumed if a dispatch or query is associated with the `via:invoke`). The resource IRI uses the `java` protocol, e.g. `java://ai.vacuity.rudi.adaptors.interfaces.IResponseModule`. The client is alerted to the arrival of the response only after the response module has completed its operations, allowing the module the opportunity to modify the response data. |                          |             |
...
```xml
	<via:TupleQuery rdf:about="#missingUserInfo">
...
WHERE {
    ...
    <via:Listener rdf:about="#createTransaction">
 		<rdfs:comment>Start using the transaction service</rdfs:comment>
      	<via:isEmpty rdf:resource="#missingUserInfo"/>
...
The security policies of the RUDI VI network are upheld by various controls within the software. Examples of these are:

1. **Anonymity** - a node keeps confidential the event history (*behavior*) and idenity of the event originator, except where the originator's explicit consent has been given to share their behavior and identity with a peer or other service. This forms a privacy boundary which extends from the sensory surface up to the physical dispatch channel endpoint. NodeIds for nodes at the user-facing sensory surface are randomly selected from a modest block of locally generated NodeIds at each event. This exploits strength in numbers, the strength of the anonymity is proportionate to the size of the network, but is susceptible to traffic analysis attacks. A [DC-Net](http://www.cs.uu.nl/docs/vakken/b3sec/Proj12/Dining.pdf) worker pool is currently being investigated to provide strong anonymity in the RUDI sensory workpool.
2. **Accountability** - with the exception of nodes at the user-facing sensory surface (those responsible for initiating the routing of an event), a worker pool node exposes the identity of its owner in a manner that is non-repudiable, as a condition to joining the worker pool. NodeIds are specific to the worker pool.
3. **Integrity** - URLs and Java Service Providers invoked by event handlers are sandboxed to an approved list managed by the node owner
