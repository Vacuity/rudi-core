...
1. **Anonymity** - a node keeps confidential the event history (*behavior*) and idenity of the event originator, except where the originator's explicit consent has been given to share their behavior and identity with a peer or other service. This forms a privacy boundary which extends from the sensory surface up to the physical dispatch channel endpoint. NodeIds for nodes at the user-facing sensory surface are randomly selected from a modest block of locally generated NodeIds at each event. This exploits strength in numbers, the strength of the anonymity is proportionate to the size of the network. A [DC-Net](http://www.cs.uu.nl/docs/vakken/b3sec/Proj12/Dining.pdf) worker pool is currently being investigated to provide strong anonymity in the RUDI sensory workpool.
