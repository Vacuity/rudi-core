...
1. **Anonymity** - a node keeps confidential the event history (*behavior*) and idenity of the event originator, except where the originator's explicit consent has been given to share their behavior and identity with a peer or other service. This forms a privacy boundary which extends from the sensory surface up to the physical dispatch channel endpoint. A [DC-Net](http://www.cs.uu.nl/docs/vakken/b3sec/Proj12/Dining.pdf) worker pool is currently being investigated to provide strong anonymity to RUDI network end-users.
2. **Accountability** - with the exception of nodes at the user-facing sensory surface (those responsible for initiating the routing of an event), a worker pool node exposes the identity of its owner in a manner that is non-repudiable, as a condition to joining the worker pool. NodeIds are specific to the worker pool. NodeIds for nodes at the user-facing sensory surface are randomly selected from a large block of locally generated NodeIds at each event.
