...
1. **Anonymity** - a node keeps confidential the event history (*behavior*) and idenity of the event propagator, except where the event propagator's explicit consent has been given to share their behavior and identity with a peer or other service. This forms a privacy boundary which extends from the sensory surface up to the physical dispatch channel. A [DC-Net](http://www.cs.uu.nl/docs/vakken/b3sec/Proj12/Dining.pdf) worker pool is currently being investigated to provide strong anonymity to RUDI network end-users.
...
3. **Integrity** - URLs and Java Service Providers invoked by third-party contributed event handlers are sandboxed to an approved list managed by the node owner
